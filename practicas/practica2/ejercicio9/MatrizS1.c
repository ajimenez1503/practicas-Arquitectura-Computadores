...
Paralelice el bucle que recorre las filas de la matriz.

Para compilar: gcc -O2 MatrizS1.c -o MatrizS1 -fopenmp
Para ejecutar use: MatrizS1 longitud
...
int main(int argc,char **argv) {
    ...
    cgt1=omp_get_wtime();
    #pragma omp parallel for private(resultado,j)
    for(i=0; i<N; i++) {
        resultado=0;
        for(j=0; j<N; j++) {
