...
en paralelo la suma de dos vectores (v3 = v1 + v2; v3(i)=v1(i)+v2(i), i=0,...N-1)
usando las directivas parallel y for. Se debe paralelizar también las tareas
asociadas a la inicialización de los vectores. Se debe obtener el tiempo ( elapsed
time) que supone el cálculo de la suma. Para obtener este tiempo usar la función
omp_get_wtime(), que proporciona el estándar OpenMP.
